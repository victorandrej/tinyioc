...
public class Configuration {
    private LinkedList<BeanConfiguration> beans = new LinkedList<>();
    private Map<Class<?>, BeanConfiguration> classBeanMap = new HashMap<>();
    ...
    public Configuration bean(Class<?> bean) {
        ...
        var configuration = new BeanConfiguration(resolveBeanName(b, bean), bean);
        classBeanMap.put(bean, configuration);

        return bean(configuration);
    }

    private Configuration bean(BeanConfiguration configuration) {
        beans.add(configuration);
        return this;
    }
    public Configuration bean(Class<?> bean, String name) {
        testClass(bean);
        return bean(new BeanConfiguration(name, bean));
    }
    ...
    public Configuration bean(Object bean) {
        ...
        return bean(new BeanConfiguration(b.beanName(), bean));
    }

    public LinkedList<BeanConfiguration> getBeans() {
        return beans;
    }
    ...
    public Map<Class<?>, BeanConfiguration> getClassBeanMap() {
        return classBeanMap;
    }
